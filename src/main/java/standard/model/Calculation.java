...
import standard.model.handler.ExpressionResolver;
import standard.model.handler.HistoryHandler;
import standard.model.handler.MemoryHandler;
...
public class Calculation {
   ...
   private void nonBinaryHandle() throws OverflowException, CannotDivideByZeroException, ResultIsUndefinedException, InvalidInputException {
      if (currentOperator == PERCENT) {
         percentResolve();
      } else if (currentOperator == NEGATE && !currentValNew) {
         resolveOperation(currentOperator);
         nonBinaryIsNew = true;
      } else {
         historyHandle(nonBinaryIsNew);
